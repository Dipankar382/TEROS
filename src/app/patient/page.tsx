...
export default function PatientPage() {
  ...
  useEffect(() => { setActiveRole('patient'); }, [setActiveRole]);

  useEffect(() => {
    if (!loading && isFirebaseReady) {
      if (!user) { router.push('/login'); return; }
      if (role && role !== 'patient' && role !== 'admin') router.push(`/${role}`);
    }
  ...
  if (loading && isFirebaseReady) {
    return (
      <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
        <Loader2 size={40} style={{ animation: 'spin 1s linear infinite', color: 'var(--critical)' }} />
        <div style={{ fontSize: '14px', color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '1px' }}>LOADING SOS INTERFACE...</div>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    );
  }

  if (isFirebaseReady && role && role !== 'patient' && role !== 'admin') {
    return (
      <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
