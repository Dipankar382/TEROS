...
export default function HospitalPage() {
  ...
  useEffect(() => { setActiveRole('hospital'); }, [setActiveRole]);

  useEffect(() => {
    if (!loading && isFirebaseReady) {
      if (!user) { router.push('/login'); return; }
      if (role && role !== 'hospital' && role !== 'admin') router.push(`/${role}`);
    }
  ...
  if (loading && isFirebaseReady) {
    return (
      <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ fontSize: '14px', color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '1px' }}>LOADING HOSPITAL PANEL...</div>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    );
  }

  if (isFirebaseReady && role && role !== 'hospital' && role !== 'admin') {
    return (
      <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
        <ShieldOff size={48} color="var(--critical)" />
        <div style={{ fontSize: '20px', fontWeight: 900 }}>ACCESS DENIED</div>
        <div style={{ fontSize: '14px', color: 'var(--text-muted)' }}>Hospital credentials required.</div>
