...
// Lazily import panels and map to reduce initial bundle
...
export default function AdminPage() {
  ...
  useEffect(() => { setActiveRole('admin'); }, [setActiveRole]);

  useEffect(() => {
    if (!loading && isFirebaseReady) {
      if (!user) { router.push('/login'); return; }
      if (role && role !== 'admin') router.push(`/${role}`);
    }
  ...
  if (isFirebaseReady && role && role !== 'admin') {
    return <AccessDenied />;
  }
...
function LoadingSpinner() {
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ fontSize: '14px', color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '1px' }}>LOADING ADMIN NEXUS...</div>
...
function AccessDenied() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-solid)', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ fontSize: '20px', fontWeight: 900, color: 'var(--text)' }}>ACCESS DENIED</div>
      <div style={{ fontSize: '14px', color: 'var(--text-muted)' }}>Admin credentials required.</div>
